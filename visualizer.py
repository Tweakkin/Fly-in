...
def draw_graph(map_file):
    ...
    drone_img_orig = pygame.image.load(os.path.join(assets_dir, "drone.png")).convert_alpha()
    drone_img      = pygame.transform.scale(drone_img_orig, (DRONE_SIZE, DRONE_SIZE))

    zone_size = NODE_RADIUS * 2
    zone_imgs = {
        "normal":     pygame.transform.scale(pygame.image.load(os.path.join(assets_dir, "normal_zone.png")).convert_alpha(),     (zone_size, zone_size)),
        "priority":   pygame.transform.scale(pygame.image.load(os.path.join(assets_dir, "normal_zone.png")).convert_alpha(),     (zone_size, zone_size)),
        "blocked":    pygame.transform.scale(pygame.image.load(os.path.join(assets_dir, "blocked_zone.png")).convert_alpha(),     (zone_size, zone_size)),
        "restricted": pygame.transform.scale(pygame.image.load(os.path.join(assets_dir, "restricted_zone.png")).convert_alpha(), (zone_size, zone_size)),
        "hub":        pygame.transform.scale(pygame.image.load(os.path.join(assets_dir, "start_end_zone.png")).convert_alpha(),  (zone_size, zone_size)),
    }
    ...
    while True:
        ...
        for connections in graph.connection_dict.values():
            for conn in connections:
                ...
                if conn.zone_1 in pos and conn.zone_2 in pos:
                    # thick dark outline + bright colored line on top
                    pygame.draw.line(screen, (40, 40, 40),   pos[conn.zone_1], pos[conn.zone_2], 9)
                    pygame.draw.line(screen, (80, 140, 220), pos[conn.zone_1], pos[conn.zone_2], 5)
                    if conn.max_capacity > 1:
                        mx = (pos[conn.zone_1][0] + pos[conn.zone_2][0]) // 2
                        my = (pos[conn.zone_1][1] + pos[conn.zone_2][1]) // 2
                        lbl = font.render(f"cap:{conn.max_capacity}", True, (20, 20, 20))
                        screen.blit(lbl, (mx, my - 16))

        # nodes
        for zone in graph.zone_dict.values():
            if zone.name not in pos: continue
            x, y = pos[zone.name]
            is_hub = (zone.name == graph.start_hub.name or
                      zone.name == graph.end_hub.name)
            if zone.type == "priority" and not is_hub:
                pygame.draw.circle(screen, (30, 80, 200),  (x, y), NODE_RADIUS)
                pygame.draw.circle(screen, (100, 160, 255), (x, y), NODE_RADIUS, 3)
            else:
                img_key = "hub" if is_hub else zone.type
                img = zone_imgs.get(img_key, zone_imgs["normal"])
                screen.blit(img, (x - NODE_RADIUS, y - NODE_RADIUS))
            lbl = font.render(zone.name, True, (10, 10, 10))
            # white outline for readability
            for ox, oy in [(-1,0),(1,0),(0,-1),(0,1)]:
                outline = font.render(zone.name, True, WHITE)
                screen.blit(outline, (x - lbl.get_width()//2 + ox, y + NODE_RADIUS + 5 + oy))
            screen.blit(lbl, (x - lbl.get_width() // 2, y + NODE_RADIUS + 5))
        ...
        # HUD
        if finished:
            hud = font_big.render(f"Done!  {turn_index} turns", True, (30, 150, 60))
        else:
            hud = font_big.render(f"Turn {turn_index} / {len(turns_data)}", True, (20, 20, 80))
